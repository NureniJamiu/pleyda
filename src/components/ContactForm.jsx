  ...
  const { values, errors, touched, handleBlur, handleChange, handleSubmit } =
    useFormik({
      ...
      onSubmit: (values, actions) =>
        onSubmit(values, actions, setIsLoading, setIsModalOpen),
    });
      ...
      <form onSubmit={handleSubmit} className="w-72">
        {["first_name", "email", "phone_number"].map((fieldName) => (
          <div key={fieldName} className="mt-6">
            <input
              ...
              className={`border border-zinc-800 bg-inherit py-2 px-5 w-full rounded text-sm focus:outline-none ${
        ...
        <div className="mt-6">
          <textarea
            ...
            rows={5}
            placeholder="Message"
            className={`border border-zinc-800 bg-inherit py-2 px-5 w-full rounded text-sm resize-none focus:outline-none ${
